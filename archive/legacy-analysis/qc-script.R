...
if (interactive()){
  ...
  sampleFile = c(
    ...
    "../../Volumes/Fl/sc_analysis/scAboslute-obj/SLX-24757_100.rds"
    
  )
  sampleName = "SLX-24757_100"
...
## Setup ====
BASEDIR="Documents/zips/"
BASEDIR=normalizePath(BASEDIR)
OUTPUT_PATH="../../Volumes/Fl/sc_analysis/post-scAbsolute/"
...
plotDataset(object=non_outlier_object, file=paste0(OUTPUT_PATH,"/",sampleName,"/non_outlier_out.pdf"), f=plotCopynumber,verbose=FALSE)



pdf(paste0(OUTPUT_PATH,"/",sampleName,"/normal_dist_alpha.pdf"), width = 3.5, height = 3, pointsize=7)
x <- df$hmm.alpha
...
condition_region <- x [log(x) > mean(log(x)) + (alpha_cutoff * sd(log(x)))]
abline(v = exp(mean(log(x)) + (alpha_cutoff * sd(log(x)))), col = "red", lwd = 1)
text(exp(mean(log(x)) + (alpha_cutoff * sd(log(x)))), par("usr")[4], labels = sprintf("exp(mean(log(x)) + (cutoff_value * sd(log(x)))).   %.2f", exp(mean(log(x)) + (alpha_cutoff * sd(log(x))))),
...
cn_non_out <- object@assayData$copynumber
...
Biobase::pData(non_outlier_object)$IsNormal <- rownames(Biobase::pData(non_outlier_object)) %in% rownames(normals)

plotCopynumberHeatmap(non_outlier_object,file =paste0(OUTPUT_PATH,"/",sampleName,"/heatmap_clustered.pdf") ,cluster_rows = Biobase::pData(non_outlier_object)$IsNormal, row_split=NULL,
