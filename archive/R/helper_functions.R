...
getCentromereDistCounts<-function(abs_profiles,centromeres,chrlen)
{
    ...
    for(i in samps)
    {
        ...
        for(c in chrs)
        {
            if(nrow(segTab)>1)
            {
                starts<-as.numeric(as.character(segTab[segTab$chromosome==c,2]))[-1]
                segstart<-as.numeric(as.character(segTab[segTab$chromosome==c,2]))[1]
                ends<-as.numeric(as.character(segTab[segTab$chromosome==c,3]))
